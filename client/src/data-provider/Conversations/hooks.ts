...
export const useConversationCost = (
  ...
  return useQuery<ConversationCostDisplay | null>(
    ...
    async () => {
      ...
      try {
        return await request.get(`/convos/${conversationId}/cost`);
      } catch (error: any) {
...
export const useConversationCostDetail = (
  ...
  return useQuery<ConversationCostSummary | null>(
    ['conversationCostDetail', conversationId],
    async () => {
      if (!conversationId) return null;

      try {
        return await request.get(`/convos/${conversationId}/cost?detailed=true`);
      } catch (error: any) {
        if (error.response?.status === 404) {
          return null; // No cost data available
        }
...
export const useMultipleConversationCosts = (
  ...
  return useQuery<MultipleConversationCosts>(
    ['multipleConversationCosts', conversationIds],
    async () => {
      if (!conversationIds.length) return {};

      try {
        return await request.post('/convos/costs', {
          conversationIds,
        });
      } catch (error) {
